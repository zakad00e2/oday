  ...
  const buildWhatsAppMsg = () => {
    ...
    if (cart.trips.length > 0) {
      ...
      cart.trips.forEach((t) => {
        const tripCost =
          t.startingPrice > 0 ? `$${t.startingPrice * guestsTotal}` : "السعر عند الطلب";
        msg += `  • ${t.titleAr} — ${tripCost}\n`;
      });
      msg += "\n";
    }
        ...
        {totalItems > 0 && (
          <div className="border-t border-[#E2E8F0] px-6 py-5 space-y-3 bg-white">
            ...
            <a
              ...
              className="w-full bg-[#25D366] hover:bg-[#20BE5C] text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all text-sm shadow-sm hover:shadow-md"
            >
