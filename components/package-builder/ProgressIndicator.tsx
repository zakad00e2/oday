...
export default function ProgressIndicator() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4">
      ...
      <div className="h-2 bg-[#E2E8F0] rounded-full mb-6 overflow-hidden">
        <div
          className="h-full bg-gradient-to-l from-[#0EA5E9] to-[#38BDF8] rounded-full transition-all duration-700 ease-in-out"
      ...
      <div className="flex justify-between">
        {steps.map((s) => {
          ...
          return (
            <div key={s.num} className="flex flex-col items-center gap-2 w-20 sm:w-28">
              <button
                onClick={() => clickable && setStep(s.num)}
                disabled={!clickable}
                className={`w-10 h-10 rounded-full border-[3px] flex items-center justify-center text-sm font-bold transition-all duration-300 ${
                  isActive
              ...
              <span
                className={`text-[11px] font-medium text-center leading-tight ${
                  isActive ? "text-[#0EA5E9] font-bold" : isCompleted ? "text-[#0EA5E9]" : "text-[#94A3B8]"
                }`}
              >
                {s.label}
              </span>
