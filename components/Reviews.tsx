...
export default function Reviews() {
  ...
  const resumeAuto = () => { autoScrollRef.current = setInterval(() => scroll("next"), 4000); };
          ...
          <div className="relative" onMouseEnter={pauseAuto} onMouseLeave={resumeAuto}>
            ...
            <div className="flex items-center justify-center gap-4 mt-8">
              ...
              <div className="flex gap-2">
                {reviews.map((_, i) => (
                  <button key={i} onClick={() => scrollToIdx(i)} className={`h-2 rounded-full transition-all duration-300 ${i === activeIdx ? "w-6 bg-[#111]" : "w-2 bg-[#D1D5DB]"}`} />
                ))}
              </div>
