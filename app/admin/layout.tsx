...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] flex" style={{ fontFamily: "'Cairo', sans-serif" }} dir="rtl">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 right-0 z-50 w-[260px] bg-white border-l border-[#E5E7EB] flex flex-col transition-transform duration-300 lg:translate-x-0 ${
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5 border-b border-[#F3F4F6]">
          <div className="flex items-center gap-2.5">
            <Image src="/optimized/logo.webp" alt="Oday Tourism" width={120} height={40} sizes="120px" quality={70} className="h-10 w-auto object-contain" />
            <div className="leading-tight">
              <p className="text-[13px] font-bold text-[#111]">Oday</p>
              <p className="text-[10px] text-[#9CA3AF]">Tourism</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="h-16 bg-white border-b border-[#E5E7EB] flex items-center justify-between px-6 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3 mr-auto">
            <button className="relative p-2 rounded-xl text-[#6B7280] hover:bg-[#F3F4F6] hover:text-[#111] transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />
              </svg>
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full" />
            </button>
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#0EA5E9] to-[#6366F1] flex items-center justify-center text-white text-xs font-bold">
              م
            </div>
          </div>
        </header>
